...
- Feat: **switching back from the openccu-loom backend to the direct-CCU backend preserves entities.** The loom→aiohomematic unique_id rewrite (`_async_restore_aiohomematic_unique_ids`) is the exact inverse of the existing aiohomematic→loom migration: it strips the `loom_` namespace and restores the hub/internal/virtual-remote central-id slot from the CCU serial suffix back to the `entry_id[-10:]` prefix, so history, area and customisations follow the entity instead of orphaning. Runs once, early in setup, idempotent and entry-scoped. The CCU↔loom round-trip is now lossless for the entity set both backends produce.
